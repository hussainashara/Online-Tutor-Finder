...
app.post("/register", async (req, res) => {
  try {
    ...
    if (usern === null) {
      ...
      res.render("register.pug", {
        msg: "Username Exist, Please Choose other",
      });
    }
  } catch (error) {
    console.log(error);
...
app.get("/search", (req, res) => {
  ...
  if (
    ...
    req.query.postalcode != null &&
    req.query.stuaddress != null &&
    req.query.searchlevel != null
  ) {
    ...
    searcharray1 = {
      subname1: subname,
      poscode1: poscode,
      studentaddress1: studentaddress,
    };
    // console.log(subname);

    Tutor.find(
      { subject: subname, pincode: poscode, searchlevel: level },
      function (err, dataa) {
        if (err) {
          ...
          res
            ...
            .render("search.pug", {
              records: [],
              message: "Pls enter details properly",
            });
        } else {
          // console.log(dataa);
          if (
            ...
            res
              ...
              .render("search.pug", {
                records: [],
                message: "Pls enter all fields",
              });
          } else if (dataa == "") {
            res
              .status(200)
              .render("search.pug", { records: [], message: "No data Found" });
          } else {
            res
              .status(200)
              .render("search.pug", { records: dataa, searcharray1 });
          }
        }
      }
    );
    ...
    req.query.postalcode == null &&
    req.query.stuaddress == null &&
    (req.query.tutorratingselect != null ||
      req.query.priceselect != null ||
      req.query.distancebtwn != null)
  ) {
            ...
            if (subname == "" || poscode == "") {
              ...
              void (async function () {
                for (var i = 0; i < dataa.length; i++) {
                  ...
                  var site =
                    "https://maps.distancematrixapi.com/maps/api/distancematrix/json?origins=" +
                    studentaddress +
                    "&destinations=" +
                    tutoraddress +
                    "&departure_time=now&key=AlphaDMAWcV4R0eBboB7z2jxtfRqm1TjNUeXKwG0";
                  // console.log(i+"-----");
                ...
                if (data2 == "") {
                  ...
                  res
                    .status(200)
                    .render("search.pug", { records: data2, searcharray1 });
                }
              })();
            }
          }
    ...
    Book.find({ student: user, tutor: uname }, function (err, data) {
      ...
      if (err) {
        ...
        if (data == "") {
          ...
          Tutor.find(
            { username: uname },
            { feedbacks: { $elemMatch: { student_username: user } } },
...
let transporter = nodemailer.createTransport({
  ...
  service: "Gmail",
  auth: {
    user: "Genral Email of your website",
    pass: "emails password",
  },
});
app.post("/forget", (req, res) => {
  ...
  // console.log(email);
  var mailOptions = {
    from: "Genral Email of your website",
    to: req.body.email,
...
app.post("/password", async (req, res) => {
  ...
  const data = await Register.findOneAndUpdate(
    { username: uname },
    { password: newpass },
    function (err, doc) {
      if (err) {
    ...
    const data = await Register.findOneAndUpdate(
      ...
      function (err, doc) {
        if (err) {
          ...
          res.render("login.pug", {
            msg: "Your Password Updated please LogIn",
          });
        }
      }
    );
...
app.post("/book", async (req, res) => {
  ...
  var mailOptions = {
    ...
    html:
      "<p>You have booked Tutor Mr/Mrs " +
      data3.firstname +
      " for the subject " +
      data3.subject +
      "having fees of Rs" +
  ...
  var mailOptions1 = {
    ...
    html:
      "<p>You have been booked by Mr/Mrs " +
      user +
      " for the subject " +
      data3.subject +
      ".</p>" +
      "<p> Please Check the Dashboard For more Info</p>",
  };
